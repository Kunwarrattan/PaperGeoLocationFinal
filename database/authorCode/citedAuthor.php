...
$query = ' SELECT `ID_Art`,`Ordre`,`Nom`,`Pyear` FROM `authors` LIMIT 9,100' ;
...
if($result->num_rows>0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $query2 = " INSERT INTO `authortable`( `authors`, `Title`,`DOP`) VALUES (\"$Nom\",$author_ID, $year)";
        ...
        if($idN == null){
            $query3 = "SELECT `id`,`authors` FROM `authortable` where `Title` = $author_ID ";
            ...
            if($flag == 0){
                $queryUpdate ="\n UPDATE `authortable` set `authors` = \"$newAuthor\" WHERE `id` = $idN";
