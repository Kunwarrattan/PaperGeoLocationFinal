...
//this file is mapping data to the Cited_addresses form address unique
...
for($index=1;$index<100000;$index=$index+100){

//for($index =1 ;$index <100 ;$index= $index+50)
//{
$query1 = ' SELECT * FROM `cited_papers_addresses` where `mapID` is NULL Limit '.($index*1).",100 ";
$result1 = mysqli_query($link, $query1);

if($result1->num_rows>0){
    while ($row = mysqli_fetch_assoc($result1)) {
        $id1 = $row['Cited_ID_Art'];
        $order = $row['ordre'];
        ...
        echo "->".$idN;
        $queryUpdate ="UPDATE `cited_papers_addresses` set `latlongID` = $latlongID , `mapID` = $idN  WHERE `Cited_ID_Art` = $id1 and `ordre` = $order ";
