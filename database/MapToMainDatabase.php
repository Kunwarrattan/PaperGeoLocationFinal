...
if($result1->num_rows>0) {
    while ($row = mysqli_fetch_assoc($result1)) {
        ...
        $sql2 = "SELECT `ordre` FROM `addresses` where `id_art`= $addFID ";
            ...
            while ($row = mysqli_fetch_assoc($result2)) {
                ...
                if ($result3->num_rows > 0) {
                    while ($row = mysqli_fetch_assoc($result3)) {
                        ...
                        $sql4 = "SELECT `ordre` FROM `cited_papers_addresses` where `Cited_ID_Art`= $addCFID ";
						echo "<br />".$sql4;
                        ...
                        if ($result4->num_rows > 0) {
                            while ($row = mysqli_fetch_assoc($result4)) {
                                ...
                                $sql5 = "SELECT `serial`,`Nom` FROM `cited_papers_authors` where `Cited_ID_Art`= $addCFID and `Ordre` = $order1 ";
